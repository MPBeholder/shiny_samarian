server <- function(input, output, session){
  ...
  buildable <- reactiveValues(logical = T)
  ...
  observeEvent(input$display_card,{
    ...
    sendSweetAlert(
      ...
      text = div(style = "width:100%;height:260px;overflow-y:scroll;",
                 fluidRow(
        column(width = 6,
               (imageOutput("stat1",height = 'auto',width = "100%")#HTML(paste0('<img class = "custom" src = "stat_cards/',gsub(" ","_",tolower(displayedCard)),'_0.png">'))
        )),
        column(width = 6,
               (imageOutput("stat2",height = 'auto',width = "100%")#HTML(paste0('<img class = "custom" src = "stat_cards/',gsub(" ","_",tolower(displayedCard)),'_1.png">'))
        ))
      )),
      type = "info"
    )
  ...
  selectedArmy <- reactive({
    ...
    session$sendCustomMessage('unbind-DT', 'ArmyTable')
    ...
    if (input$army_selection == "Dragyri") {
      ...
      dragLevels <- c("Air","Earth","Fire","Ice","Shadow","Unaligned")
      dragArrange <- c("Bounty Hunters",input$subfaction_selection,dragLevels[-which(dragLevels == input$subfaction_selection)])
      
      dragTrueborn <- faction.Df  %>%
        dplyr::filter(grepl('Dragyri', Faction)) %>%
        dplyr::filter(Subfaction != input$subfaction_selection) %>%
        dplyr::filter(Name %in% trueBorns$Name)
    ...
    currentArmy <- currentArmy %>%
      ...
      mutate(Icon = case_when(
        Subfaction %in% c("Fire","Ice","Earth",
                          "Shadow","Air","Bounty Hunter",
                          "Prevailer","Isaac","Joan",
                          "Heretic","John","Luke",
                          "Mark","Mary",
                          "Blood","Decay",
                          "Metamorphosis",
                          ...
                          "Delta Broodfolk",
                          "Progeny"
                          ) ~ paste0('<img height = "40px" src="',switch(Subfaction,
                                                         "Fire" = "faction_icons/logo_dragyri_fire.png",
                                                         "Ice" = "faction_icons/logo_dragyri_ice.png",
                                                         "Earth" = "faction_icons/logo_dragyri_earth.png",
                                                         "Shadow" = "faction_icons/logo_dragyri_shadow.png",
                                                         "Air" = "faction_icons/logo_dragyri_air.png",
                                                         "Bounty Hunter" = "faction_icons/logo_bounty_hunters.png",
                                                         "Prevailer" = "faction_icons/logo_forsaken_prevailers.png",
                                                         ...
                                                         "Luke" = "faction_icons/logo_forsaken_saint_luke_0.png",
                                                         "Mark" = "faction_icons/logo_forsaken_saint_mark.png",
                                                         "Mary" = "faction_icons/logo_forsaken_saint_mary.png",
                                                         "Blood" = "faction_icons/logo_skarrd_blood.png",
                                                         "Decay" = "faction_icons/logo_skarrd_decay.png",
                                                         "Metamorphosis" = "faction_icons/logo_skarrd_metamorphosis.png",
                                                         "Toxic" = "faction_icons/logo_skarrd_toxic.png",
                                                         "Saltflat Nomads" = "faction_icons/logo_outcasts_salt_flats_nomads.png",
                                                         "Barrow Slavers" = "faction_icons/logo_outcasts_slavers.png",
                                                         "Broodmere Spawn" = "faction_icons/broodspawn_front.png",
                                                         "Delta Broodfolk" = "faction_icons/delta_front.png",
                                                         "Progeny" = "faction_icons/progeny_front.png"),
                                                         '"></img>'),
        TRUE ~ paste0('<img height = "40px" src="',switch(input$army_selection,
                                                          "Dragyri" = "faction_icons/logo_dragyri.png",
                                                          "Kukulkani" = "faction_icons/logo_kukulkani.png",
                                                          "Forsaken" = "faction_icons/logo_forsaken.png",
                                                          "Outcasts" = "faction_icons/logo_outcasts.png",
                                                          "C.O.R.E" = "faction_icons/logo_core.png",
                                                          "Brood" = "faction_icons/logo_brood_0.png",
                                                          "Skarrd" = "faction_icons/logo_skarrd.png"),
                      '"></img>')
      )) %>%
      group_by(Name,Subfaction)
    #tempCurrentArmy <<- currentArmy
    if (input$army_selection == "Dragyri") {

      currentArmy <- currentArmy %>%
        arrange(match(Subfaction, dragArrange), Amount) %>%
        ungroup() %>%
        group_by(Name) %>%
        ...
        dplyr::select(Faction,Icon,Subfaction,Name,Display,Cost,Amount,Allotment,Number)
  ...
  output$ArmyTable <- renderDataTable((selectedArmy()), selection = 'none',
                                      ...
                                      options = list(dom = 't',
                                                     ...
                                                     scrollX = TRUE,
                                                     paging = FALSE,
                                                     columnDefs = list(list(targets = {c(4,8)},
                                                                            orderable = FALSE),
                                                                       list(targets = {c(0,2,6,7)},
                                                                            visible = FALSE)
                                                                       ),
                                                     scrollY = '500px',
  ...
  output$Downloader <- renderUI({
    ...
    validate(
      ...
      need(buildable$logical, "Too many Non-Caste Dragyri!")
    ...
    content = function(file) {
      ...
      outputArmy <- outputArmy %>%
        ...
        mutate(Amount = case_when(
          Name %in% c("Charity's_Zeal","Charity's_Might","Dominique's_Chalica","Pud_Roamer","Aether_Lord","Aether_Wisp") ~ "Special",
          TRUE ~ Amount
        ))
  ...
  # Add-Ons : Extra Download Confirmation ---------------------------------
    ...
    tagList(
      lapply(armyNames, function(x) {
        ...
        formattingArmy <- selectedArmy %>%
          dplyr::select(-User,-SavedName,-actualScore,-armySubfaction) %>%
          mutate(Name = gsub("_"," ",Name)) %>%
          ...
          mutate(Icon = case_when(
            Subfaction %in% c("Fire","Ice","Earth",
                              ...
                              "Progeny"
                                                           ...
                                                           "Progeny" = "faction_icons/progeny_front.png"),
                       '"></img>'),
            TRUE ~ paste0('<img height = "40px" src="',switch(Faction,
                                                              "Dragyri" = "faction_icons/logo_dragyri.png",
                                                              "Kukulkani" = "faction_icons/logo_kukulkani.png",
                                                              "Forsaken" = "faction_icons/logo_forsaken.png",
                                                              "Outcasts" = "faction_icons/logo_outcasts.png",
          ...
          dplyr::select(Icon,Name,Type,Value,Amount,`Upgrades & Bio-Gens`,`Psychogenics & Rituals`)
        ...
        if (length(droppedSub) == 0) {
          ...
          armyIcon <- switch(mainArmy,
                             ...
                             "Skarrd" = "faction_icons/logo_skarrd.png","Outcasts" = "faction_icons/logo_outcasts.png",
          ...
          armyIcon <- switch(droppedSub,
                             ...
                             "Progeny" = "faction_icons/progeny_front.png","Blood" = "faction_icons/logo_skarrd_blood.png",
                             "Decay" = "faction_icons/logo_skarrd_decay.png","Metamorphosis" = "faction_icons/logo_skarrd_metamorphosis.png",
                             "Toxic" = "faction_icons/logo_skarrd_toxic.png","Mary" = "faction_icons/logo_forsaken_saint_mary.png",
                             "Isaac" = "faction_icons/logo_forsaken_saint_isaac.png",
                             "Luke" = "faction_icons/logo_forsaken_saint_luke_0.png",
                             "Joan" = "faction_icons/logo_forsaken_saint_joan.png",
                             "Heretic" = "faction_icons/logo_forsaken_saint_johann.png",
  ...
  observeEvent(input$subfaction_selection,{
    if (input$subfaction_selection %in% c("Blood","Metamorphosis","Toxic","Decay",
      ...
      armyText <- switch(input$subfaction_selection,
                         "Air" = "As quick and ephemeral as their namesake, the Air Caste dodges their opponent's attacks while slicing into their foes like the wind from a hurricane.<br>A Dragyri force may select up to 150 points of Dragyri Trueborns from different Elemental Castes.",
                         "Earth" = "As steady as the ground we stand upon, the Earth Caste is adept at controlling the field of battle itself.<br>A Dragyri force may select up to 150 points of Dragyri Trueborns from different Elemental Castes.",
                         "Fire" = "Filled with rage, the Fire Caste can set their foes aflame with their blows.<br>A Dragyri force may select up to 150 points of Dragyri Trueborns from different Elemental Castes.",
                         "Ice" = "The Ice Caste is an inexorable force on the battlefield, relying on their high HP and armor to outlast their opponents.<br>A Dragyri force may select up to 150 points of Dragyri Trueborns from different Elemental Castes.",
                         "Shadow" = "With a slew of debuff and trickery abilities, the Shadow Caste disables their enemies before moving in for the kill.<br>A Dragyri force may select up to 150 points of Dragyri Trueborns from different Elemental Castes.",
  ...
  # Add-Ons : Extra JS Functionality ---------------------------------
  ...
  observe({
    ...
    req(input$army_value != 0,input$army_selection,input$subfaction_selection)
    req(selectedArmy())
    ...
    if (input$army_selection == "Dragyri") {
      ...
      if (input$subfaction_selection == "Fire" && !is.null(input[["SoulWarden(Fire)"]])) {
        #(as.numeric(input[["Rath'Zhi"]]))
        #print(as.numeric(input[["Jza'Mhi"]]))
        if (!is.null(input[["Rath'Zhi"]])) {
        if (as.numeric(input[["Rath'Zhi"]]) == 1) {
          shinyjs::delay(20,{
          shinyjs::disable(selector = paste0("#JzaMhi > select"))
          })
        } else if (as.numeric(input[["Jza'Mhi"]]) == 1) {
          shinyjs::delay(20,{
          shinyjs::disable(selector = paste0("#RathZhi > select"))
          })
        } else {
          shinyjs::delay(20,{
          shinyjs::enable(selector = paste0("#JzaMhi > select"))
          shinyjs::enable(selector = paste0("#RathZhi > select"))
          })
        }
      }
  ...
  # Add-Ons : Arbiter Commands Dragyri Functionality ---------------------------------
  
  observe({

    buildable$logical <- T
    
    req(input$army_value != 0,input$army_selection == "Dragyri",input$subfaction_selection)
    req(selectedArmy())
    arbiterCommands <- trueBorns %>%
      filter(Subfaction != input$subfaction_selection)
    
    val <- 0
    selectedNames <- tibble(Name = character(),Num = character())
    # Calculate amount of points selected
    for (name in arbiterCommands[["Name"]]){
      
      val <- val + as.numeric(input[[gsub(" ","",name)]]) * as.numeric(selectedArmy()$Cost[which(selectedArmy()[["Name"]] == name)])
      
      if (!is.null(input[[gsub(" ","",name)]])) {
        if (as.numeric(input[[gsub(" ","",name)]]) >= 1) {
          selectedNames <- rbind(selectedNames,tibble(Name = name,Num = as.numeric(input[[gsub(" ","",name)]])))
        }
      }
    ...
    if (length(val) != 0) {
      remainingTrueborns <- arbiterCommands %>%
        mutate(selectable = case_when(
          Name %in% selectedNames$Name ~ T,
          as.numeric(Cost) <= (150 - val) ~ T,
          T ~ F
        ))
      asdfRemainder <<- remainingTrueborns
      for (disableOptions in remainingTrueborns %>% pull(Name)){
        if (remainingTrueborns %>% filter(Name == disableOptions) %>% pull(selectable)){
          shinyjs::enable(selector = paste0("#",str_replace_all(disableOptions, "[[:punct:]]| ", "")," > select"))
        } else {
          shinyjs::disable(selector = paste0("#",str_replace_all(disableOptions, "[[:punct:]]| ", "")," > select"))
        }
        
        #shinyjs::disable(selector = paste0("#",str_replace_all(disabled, "[[:punct:]]| ", "")," > select"))
      }
      
      if (val > 150) {
        buildable$logical <- F
        sendSweetAlert(session,
                       title = "Too Many Non-Caste",
                       text = "You have too many Dragyri outside of your elemental caste!",
                       type = "warning")
      } else {
        buildable$logical <- T
      }
  ...
  observeEvent(selectedArmy(),{
    
    req(input$army_value != 0,input$army_selection == "Dragyri",input$subfaction_selection)
    
    if (input$subfaction_selection %in% c("Fire","Ice","Earth","Shadow","Air")) {
      #print("I Have Updated")
      arbiterOthers <- trueBorns %>%
        filter(Subfaction != input$subfaction_selection)
      
      shinyjs::delay(120,{
      for (trueborn in arbiterOthers$Name) {
        choices <- as.character(seq(0,arbiterOthers %>%
                        filter(Name == trueborn) %>% 
                        pull(Allotment)))
        updateSelectInput(session,inputId = gsub(' ','',trueborn),choices = choices)
      }
      })
    }
  },priority = -11)
